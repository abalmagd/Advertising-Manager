...
public class RegisterActivity extends AppCompatActivity
{
        ...
        if(bio.isEmpty() || name.isEmpty() || email.isEmpty() || password.isEmpty()) {
            ...
            if(email.matches(emailRegex)) {
                if(password.length() >= 5) {
                    makeRegisterReq("https://crew-project.herokuapp.com/advertisers/register");
                } else
    ...
    private void makeRegisterReq(String url)
    {
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.POST, url, new JSONObject(postParam), response -> {
